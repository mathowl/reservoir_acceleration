...
@nb.njit()
def nb_matmul( A, b): #diagonals are zero
...
def main(N):
    ...
    print(t_dur)
    with shelve.open('shelve/perform_numba-vanilla.shelve', 'c') as shelf:
        shelf[str(time.time())] = [N,t_dur]

    with shelve.open('shelve/sol_numba-vanilla.shelve', 'c') as shelf:
        shelf[str(N)] = Y
...
if __name__ == "__main__":
    with shelve.open('shelve/perform_numba-vanilla.shelve') as shelf:
        shelf.clear()
